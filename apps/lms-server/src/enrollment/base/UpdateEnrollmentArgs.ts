/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { EnrollmentWhereUniqueInput } from "./EnrollmentWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { EnrollmentUpdateInput } from "./EnrollmentUpdateInput";

@ArgsType()
class UpdateEnrollmentArgs {
  @ApiProperty({
    required: true,
    type: () => EnrollmentWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => EnrollmentWhereUniqueInput)
  @Field(() => EnrollmentWhereUniqueInput, { nullable: false })
  where!: EnrollmentWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => EnrollmentUpdateInput,
  })
  @ValidateNested()
  @Type(() => EnrollmentUpdateInput)
  @Field(() => EnrollmentUpdateInput, { nullable: false })
  data!: EnrollmentUpdateInput;
}

export { UpdateEnrollmentArgs as UpdateEnrollmentArgs };
